...
from src.link import MatrixDiagTriuLink

loc_link = IdentityLink()
loc_link._valid_structures = ["square_matrix", "matrix", "vector"]  # Still necessary
scale_link = MatrixDiagTriuLink(diag_link=LogLink(), triu_link=IdentityLink())


class MultivariateNormalInverseCholeskyNewtonRapson(Distribution):
    ...
    # The links on this cannot be changed since we have them incorporated in the derivatives.
    ...
    def set_initial_guess(self, theta: Dict, param: int):
        return theta
    ...
    def element_score(self, y: np.ndarray, theta: Dict, param: int = 0, k: int = 0):
        ...
        if param == 1:
            ...
            if ii[k] == jj[k]:
                deriv = _score_inv_chol_diag(y, fitted_loc, fitted_inv_tr_chol, ii[k])
            else:
    ...
    def element_hessian(self, y: np.ndarray, theta: Dict, param: int = 0, k: int = 0):
        ...
        if param == 1:
            ii, jj = np.triu_indices(d)
            if ii[k] == jj[k]:
                deriv = _hessian_inv_chol_diag(y, fitted_loc, fitted_inv_tr_chol, ii[k])
            else:
                deriv = _derivative_2nd_inv_chol_triu(
                    y, fitted_loc, fitted_inv_tr_chol, ii[k], jj[k]
                )
        return np.clip(deriv, -np.inf, -1e-4)

    def element_dl1_dp1(self, y: np.ndarray, theta: Dict, param: int = 0, k: int = 0):
        fitted_loc, fitted_inv_tr_chol = self.theta_to_params(theta)
        d = y.shape[1]
        if param == 0:
            deriv = _derivative_1st_mu(
    ...
    def element_dl2_dp2(
        self, y: np.ndarray, theta: Dict, param: int = 0, k: int = 0, clip=True
    ):
        fitted_loc, fitted_inv_tr_chol = self.theta_to_params(theta)
        d = y.shape[1]
        if param == 0:
...
@nb.jit()
def _score_inv_chol_diag(y, fitted_loc, fitted_inv_tr_chol, i):
    y_centered = y - fitted_loc
...
@nb.jit()
def _score_chol(y, fitted_loc, fitted_inv_tr_chol):
    n = y.shape[0]
    d = y.shape[1]
    dim = ((d + 1) * d) // 2
    k = 0
    ...
    for i in range(d):
        for j in range(i, d):
            if i == j:
                deriv[:, k] = _score_inv_chol_diag(y, fitted_loc, fitted_inv_tr_chol, i)
            else:
...
@nb.jit()
def _hessian_inv_chol_diag(y, fitted_loc, fitted_inv_tr_chol, i):
    y_centered = y - fitted_loc
...
@nb.jit()
def _hessian_inv_chol(y, fitted_loc, fitted_inv_tr_chol):
    n = y.shape[0]
    d = y.shape[1]
    dim = ((d + 1) * d) // 2
    k = 0
    ...
    for i in range(d):
        for j in range(i, d):
            if i == j:
                deriv[:, k] = _hessian_inv_chol_diag(
                    y, fitted_loc, fitted_inv_tr_chol, i
                )
            else:
                deriv[:, k] = _derivative_2nd_inv_chol_triu(
